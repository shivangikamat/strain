...
# Package lives at repo_root/emotiscan/
_REPO_ROOT = Path(__file__).resolve().parents[1]

# Primary Kaggle export: place at repo_root/data/emotions.csv (override with env).
EMOTIONS_CSV = Path(
    os.environ.get("EMOTISCAN_EMOTIONS_CSV", _REPO_ROOT / "data" / "emotions.csv")
)

# DREAMER: raw MATLAB bundle (Zenodo / IEEE DataPort).
DREAMER_MAT = Path(os.environ.get("EMOTISCAN_DREAMER_MAT", _REPO_ROOT / "data" / "raw" / "DREAMER.mat"))

# Preprocessed sliding-window tensors + labels (see scripts/export_dreamer_epochs.py).
DREAMER_PROCESSED_DIR = Path(
    os.environ.get("EMOTISCAN_DREAMER_PROCESSED", _REPO_ROOT / "data" / "processed" / "dreamer")
)
