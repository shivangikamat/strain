...
Follow these in order so **emotiscan-tools** runs locally (Cursor) and optionally on the network (Prompt Opinion + ngrok).
...
- **Kaggle / CSV tools:** place [`data/emotions.csv`](../data/emotions.csv) (or set `EMOTISCAN_EMOTIONS_CSV`) and train the classifier:
...
| Field | Value |
|--------|--------|
| **Name** | `emotiscan-tools` |
| **Command** | Full path to `.venv/bin/python` in this repo |
| **Args** | `-m`, `mcp_server.server` |
| **Cwd** | Repository root |

Set env `EMOTISCAN_MCP_TRANSPORT=stdio` if you ever changed the default elsewhere.

Restart Cursor or reload MCP, then open the MCP panel and confirm **emotiscan-tools** lists tools (e.g. `load_dataset_tool`).
...
5. Keep **`EMOTISCAN_MCP_RELAX_DNS=1`** behind ngrok so the tunneled `Host` header is accepted (development only).
...
| `FileNotFoundError` on tools | `data/emotions.csv` and/or `emotiscan/models/baseline_pipeline.joblib` missing — complete Step 2. |
| SSE / ngrok 403 or connection closed | Set `EMOTISCAN_MCP_RELAX_DNS=1`; confirm ngrok URL includes the correct path (`/sse`). |
