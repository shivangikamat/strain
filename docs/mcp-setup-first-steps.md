...
| ngrok v3 YAML: `field strain-mcp not found in type config.Endpoint` | v3 **`endpoints`** must be a **list** of objects with **`name`** and **`upstream.url`**, not `strain-mcp: http`. Example: `- name: strain-mcp` then `upstream: { url: http://127.0.0.1:8765 }`. See [ngrok agent config v3 — endpoint definitions](https://ngrok.com/docs/agent/config/v3#endpoint-definitions). Then `ngrok start strain-mcp strain-vite` or `ngrok start --all`. |
| Prompt Opinion **“An unexpected error has occurred”** (BYO) | Often **MCP or model** — not a user-visible stack trace. Check: (1) MCP tunnel points to **8765** (or your `FASTMCP_PORT`), path **`/sse`** or **`/mcp`** matches transport; (2) `STRAIN_MCP_RELAX_DNS=1` behind ngrok; (3) **Po model** under Configuration → Models; (4) **ngrok web UI** (`127.0.0.1:4040` / `4041`) for failing requests; (5) terminal running **`./scripts/run_mcp_sse.sh`** for Python tracebacks. If a tool fails, the server returns JSON with **`strain_mcp_tool_failed`** — read **`message`** / **`traceback_tail`**. **If errors started after Markdown reports:** Po may require **JSON** tool results — `patient_screening_markdown_report_tool` returns `{ "markdown": "..." }`; have the agent render the **`markdown`** field, not the raw tool string. |
